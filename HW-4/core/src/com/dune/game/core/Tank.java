...
public class Tank extends GameObject implements Poolable {
    ...
    public void setup(Owner ownerType, float x, float y) {
        ...
        this.destination2 = new Vector2(position);
      //  this.container = 0;
    }
    ...
    public void update(float dt) {
       ...
       // if (Gdx.input.isButtonPressed(Input.Buttons.LEFT)){
         //   destination2.set(Gdx.input.getX(), 720 - Gdx.input.getY());
    ...
    public void updateWeapon(float dt){
            if (weapon.getType() == Weapon.Type.HARVEST) {
                if (gc.getMap().getResourceCount(this) > 0) {
                    ...
                    if (result > -1 && container < MAXCAPACITY) {
    ...
    public void render(SpriteBatch batch) {
        ...
        if (weapon.getType() == Weapon.Type.HARVEST && weapon.getUsageTimePercentage() > 0.0f && container != MAXCAPACITY ) {
            ...
            batch.draw(progressbarTexture, position.x - 30 , position.y + 62, 60 * container / MAXCAPACITY, 8);
